...
document.getElementById("safari-form").addEventListener("submit", function (e) {
  ...
  if (!safariDate) {
    showMessage(
      "safari-message",
      "Please select a date for your safari booking.",
      false
    );
    return;
  }
  ...
  if (!safariType) {
    showMessage(
      "safari-message",
      "Please select a type for your safari booking.",
      false
    );
    return;
  }
  ...
  if (
    !isVip &&
    (safariType === "Herbivore Tour with Feeding" ||
      safariType === "T-Rex Rumble eXtreme Thrill Pack")
  ) {
    showMessage(
      "safari-message",
      "Only VIP users can book the selected safari option.",
      false
    );
    return;
  }
  ...
  const hasVipTicket = cart.some(
    (item) => item.itemType === "ticket" && item.category === "VIP"
  );
  if (!hasTicket) {
    showMessage(
      "safari-message",
      "You must purchase a general admission ticket before booking a safari.",
      false
    );
    return;
  }
  ...
  if ((day === 0 || day === 6) && !hasVipTicket) {
    showMessage(
      "safari-message",
      "VIP tickets required to book safaris on weekends.",
      false
    );
    return;
  }
  ...
  if (selectedDate < today) {
    showMessage(
      "safari-message",
      "You cannot book a safari for a date in the past.",
      false
    );
    return;
  }
...
document
  ...
  .addEventListener("click", function () {
    ...
    if (!hasAdmissionTicket) {
      alert(
        "You must have at least one admission ticket in your cart to complete checkout. Safari experiences cannot be purchased without admission tickets."
      );
      return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  for (const safariType in safariPrices) {
    if (safariPrices.hasOwnProperty(safariType)) {
      // Safest way to loop through objects
  ...
  document
    .querySelectorAll("nav a[data-section], .card-button")
    .forEach((link) => {
...
>>>>>>> origin/Dev_Branch:webpages/app.js
